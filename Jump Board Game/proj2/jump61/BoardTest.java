...
public class BoardTest {
    ...
    public void winnerTest() {
        Board one = new Board(3);
        for (int i = 0; i < one.getContent().length; i++) {
            for (int k = 0; k < one.getContent()[i].length; k++) {
                one.set(i + 1, k + 1, 1, RED);
            }
        }
        assertEquals(RED, one.getWinner());
    }
    ...
    public void newBoardTest() {
        ...
        for (int i = 0; i < one.getContent().length; i++) {
            for (int k = 0; k < one.getContent()[i].length; k++) {
                assertEquals(1, one.getContent()[i][k].getSpots());
            }
        }
    }
    ...
    public void boardCopyTest() {
        Board one = new Board(4);
        one.addSpot(RED, 2, 2);
        one.addSpot(BLUE, 3, 3);
        one.addSpot(RED, 2, 2);
        one.addSpot(BLUE, 3, 3);
        one.addSpot(RED, 2, 2);
        Board two = new Board(one);
        assertEquals(two.get(2, 2).getSpots(), 4);
    }


    @Test
    public void numNeightborTest() {
        Board a = new Board(6);
        assertEquals(a.numNeighbors(1, 6), 2);
        assertEquals(a.numNeighbors(1, 1), 2);
        assertEquals(a.numNeighbors(6, 1), 2);
        assertEquals(a.numNeighbors(6, 6), 2);
        assertEquals(a.numNeighbors(5, 1), 3);
        assertEquals(a.numNeighbors(6, 2), 3);
        assertEquals(a.numNeighbors(4, 5), 4);
    ...
    public void undoCheck() {
        ...
        a.addSpot(BLUE, 3, 3);
        a.addSpot(RED, 2, 2);
        assertEquals(a.get(2, 2).getSpots(), 4);
        a.undo();
        assertEquals(a.get(2, 2).getSpots(), 3);
