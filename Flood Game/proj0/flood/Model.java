...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        if (place.row == 0 && place.col == 0) {
        ...
        } else if (place.row == _height - 1 && place.col == _width - 1) {
            action.accept(pl(place.row - 1, place.col));
            action.accept(pl(place.row, place.col - 1));
        } else if (place.row == _height - 1 && place.col < _width - 1) {
            action.accept(pl(place.row - 1, place.col));
            action.accept(pl(place.row, place.col - 1));
            action.accept(pl(place.row, place.col + 1));
        } else if (place.col == _width - 1 && place.row < _height - 1) {
            action.accept(pl(place.row, place.col - 1));
            action.accept(pl(place.row + 1, place.col));
            action.accept(pl(place.row - 1, place.col));
        } else {
    ...
    boolean[] colorsPresent() {
        boolean[] haha = new boolean[_ncolors];
        for (int c = 0; c < _ncolors; c++) {
            haha[c] = false;
        }
        for (int row = 0; row < _height; row++) {
            for (int col = 0; col < _width; col++) {
                haha[_cells[row][col]] = true;
            }
        }
        return haha;
    }
    ...
    final boolean solved() {
        if (_active.size() == _height * _width) {
            return true;
        }
        return false;
    }
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        if (!(result.contains(start))) {
            result.add(start);
            forNeighbors(start, (place) -> {
                if (this.get(place) == color && !result.contains(place)) {
                    findRegion(place, color, result);
                }
            });
        }
    ...
    void setActiveRegionColor(int newColor) {
        HashSet<Place> newVersion = new HashSet<>();
        for (Place i : _active) {
            _cells[i.row][i.col] = newColor;
            findRegion(i, newColor, newVersion);
        }
        _current = _current + 1;
        _active.addAll(newVersion);
        _history.add(new GameState());
        _history.get(_current).saveState();
        _lastHistory = _history.size();
        clearMarks();
    }
    ...
    Set<Place> adjacentCells(int color) {
        HashSet<Place> sameColor = new HashSet<>();
        for (Place i : _active) {
            sameColor = findRegion(i, color, sameColor);
        }
        sameColor.removeAll(_active);
        return sameColor;
    }
